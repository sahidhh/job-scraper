...
**Update (scoring-accuracy session, 2026-07-17):** a prerequisite pass shipped first, deliberately *not* touching this blend — `docs/decisions.md` AD-44 added a deterministic eligibility pre-filter (`classifyEligibility.ts`) and a constraint-aware AI system prompt (candidate location/sponsorship, experience, primary/secondary stack) so `ai_score` itself stops rating location/seniority/stack mismatches as "strong". Any 30-job eval set for this PD-2 blend should be built **against the post-AD-44 prompt**, not the old one — labeling against the old, unconstrained prompt's score distribution would validate a blend weight against since-changed scoring behavior. Recommend re-confirming this eligibility filter + prompt pass actually holds up (via a live re-score once DB/OpenRouter credentials are available in-session — this session had neither, see AD-44's diagnosis) before investing in the eval set.
