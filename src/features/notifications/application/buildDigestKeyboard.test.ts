...
const WORTH_URL = "https://app.example.com/api/telegram/worth-reviewing?msg=abc&token=xyz";
...
describe("buildDigestKeyboard", () => {
  ...
  it("appends a Worth Reviewing button when count > 0 and URL is provided", () => {
    const rows = buildDigestKeyboard([], 3, { worthReviewingUrl: WORTH_URL });
    expect(rows).toHaveLength(1);
    expect(rows[0]![0]).toEqual({ text: "✓ Worth Reviewing (3)", url: WORTH_URL });
  });

  it("omits Worth Reviewing button when count is 0", () => {
    const rows = buildDigestKeyboard([], 0, { worthReviewingUrl: WORTH_URL });
    const texts = rows.flat().map((b) => (b as { text: string }).text);
    expect(texts).not.toContain(expect.stringContaining("Worth Reviewing"));
  });

  it("omits Worth Reviewing button when URL is not provided", () => {
