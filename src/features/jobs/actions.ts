...
import type { CreateStatusInput, Job, JobStatus, UpdateStatusInput } from "@/features/jobs/domain/types";
...
export async function getCompanyHistoryAction(companyName: string): Promise<ActionResult<Job[]>> {
  try {
    const client = await createSupabaseServerClient();
    const jobRepository = new SupabaseJobRepository(client);
