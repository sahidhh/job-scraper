...
**Transaction boundaries:** `create()` must (a) set any existing `is_active = true` row to `false`, then (b) insert the new row with `is_active = true` — in that order, so the partial unique index (`resumes_single_active_uq`) is never violated. Implemented as a **Postgres function** `set_active_resume(p_file_path text, p_parsed_text text, p_skills text[], p_content_hash text) returns setof resumes` called via `supabase.rpc()`, so both steps run atomically in the database (the Supabase JS client has no multi-statement transaction API). Must stay `setof` (not a bare `resumes` composite) — a non-`setof` return serializes as a JSON object, not an array, breaking `create()`'s `data?.[0]` unwrap even though the insert itself succeeds (AD-39).

**`ResumeStorage` port (`features/resume/domain/ResumeStorage.ts`, AD-40):** a separate, smaller port for the Supabase Storage side of a fresh upload (`upload(filePath, buffer, mimeType)`, `remove(filePath)`), implemented by `SupabaseResumeStorage`. Not part of `ResumeRepository` — `restoreResumeVersion` and `applyResumeSuggestions` both call `ResumeRepository.create()` without touching Storage (they reuse an existing file or have no backing file at all), so "upload the source file" is specific to `uploadResume()`, not to every new resume version. `uploadResume()` calls `resumeStorage.upload()` only after parsing/validation succeed, and calls `resumeStorage.remove()` if the subsequent `resumeRepository.create()` call fails — see AD-40 for why (a parse or DB-insert failure must never strand a Storage object with no corresponding row).
