...
### 1.1b Remotive and Himalayas (remote/visa-sponsorship refocus, AD-47)
Both are remote-global boards fetched once per run with no server-side keyword search (Remotive advises sparse fetching; Himalayas' `search` param is a verified no-op), so role filtering is client-side — the same completeness-vs-request-volume tradeoff as the other feed sources. **Himalayas considers only its most-recent page per run** (`limit=1000`, `offset=0`): if it ever posts more than ~1000 relevant jobs between two scrape runs, the overflow is not seen until they age into that window — acceptable for a personal single-user cadence, not a completeness guarantee. Both boards expose only a plain location/restriction string, normalized to a `"Remote - <X>"` `locationRaw`; single-country-locked postings (e.g. "Remote - Netherlands") are tagged `remote` at scrape time but then hard-excluded by `classifyEligibility` (§3.11 / AD-46). Region-name qualifiers those boards use ("Americas", "Europe", "EMEA") are subject to the same "not parsed" gap as §3.11 — a region that happens to exclude India is not always caught, so a few India-inapplicable remote rows can still reach scoring (where the AI prompt caps them).

### 1.1c Claude-Routine (Manual) Matches (docs/tasks/manual-job-matches.md, AD-65)
`claude_routine` is a manual-trigger-only source, same category as `careers_url` -- rows are written only by `npm run import:manual-matches -- <state.json>`, never by any cron. `minAiScore` has no effect combined with `origin=claude_routine` on `/dashboard`: those rows never have a `job_scores` row (the automated pipeline never scores them), so a `minAiScore` filter forces the inner join used elsewhere and returns zero rows rather than silently ignoring the filter. This is accepted rather than special-cased in the UI -- add a per-origin filter-hiding rule only if it proves confusing in practice.
...
PDF and DOCX resumes are supported (via pdfjs-dist and mammoth respectively). Plain text, HTML, or other formats are not accepted. A scanned/image-only PDF (no embedded text layer) is rejected with an error rather than silently creating a resume with no extracted skills.

### 2.1a Parse-Once Cache
Re-uploading a file with identical bytes (by sha256 hash) reuses the previously parsed text instead of re-running pdfjs-dist/mammoth — this only skips the parse step; a new resume version row is still created on every upload.
...
### 3.10 The Stored Eligibility Verdict Is An Ingest-Time Snapshot (`docs/decisions.md` AD-51)
**Resolved and replaced.** This section previously recorded that hard-excluded jobs were indistinguishable from AI-call failures on the row (AD-44): `classifyEligibility()`'s exclusions left `job_scores.ai_score` null with no other marker, so the dashboard reported them as "awaiting AI review" forever, and `findUnscored` re-queued them on every cron run. AD-51 fixed both by persisting the verdict as `jobs.ineligible_reason` at ingest.

The remaining limitation is staleness. `ineligible_reason` is written once, when the job is ingested, from the phrase lists in `shared/config/candidate-constraints.ts`. Editing those lists (or the matching logic in `classifyEligibility.ts`) does **not** reclassify jobs already stored — same "no retroactive recomputation" shape as §3.5/§3.8/§3.9. `npm run backfill:eligibility` is the refresh path: it re-classifies every active job and is safe to re-run at any time. A stale verdict is silently wrong in both directions: a job that should now be excluded stays visible and keeps consuming AI budget, and a job that should now be eligible stays hidden behind the default-on dashboard filter and is never queued for scoring.

### 3.11 Remote Geo-Lock Detection Covers Two Shapes Only, Not Every Location String (`docs/decisions.md` AD-46)
`classifyEligibility()`'s remote-country-lock check only fires on (1) explicit phrases in `GEO_LOCK_EXCLUSION_PHRASES`, and (2) `locationRaw` matching the structural leading-token "Remote - &lt;place&gt;" / "Remote (&lt;place&gt;)" pattern against a curated, non-exhaustive `REMOTE_SINGLE_COUNTRY_LOCK_NAMES` list (~40 countries; found gap: a real production job, `"Remote - Poland"`, initially passed shape (1) undetected until this was added). Deliberately NOT covered: region names ("EMEA", "APAC", "LATAM") — ambiguous whether they include India, so treated as open rather than risk a false exclusion; free-form multi-location strings that don't start with the literal word "Remote" (e.g. `"San Francisco, CA, New York, NY, ... or Remote within Canada or United States"`) — too unstructured to regex-parse safely; and any country not in the curated list (a locationRaw naming an uncommon country not yet added will pass through as eligible). A job matching one of these uncovered shapes still reaches stage 2 and is scored by the constraint-aware prompt (`docs/scoring.md` §3), which may or may not independently catch the geography in its reasoning — as seen in the Poland case pre-fix, where the AI's own seniority/stack-mismatch reasoning happened to produce the correct low score anyway, but is not a reliable substitute for the deterministic filter.

### 3.12 Dashboard Ranking Is Sorted In Memory, Bounded by `DASHBOARD_FETCH_CAP` (`docs/decisions.md` AD-49)
The dashboard's "sort by overall score" is applied in application code, not by the database: PostgREST cannot order the parent `jobs` rows by the embedded one-to-many `job_scores.overall_score` column, so `findForDashboard` fetches the filtered set (capped at `DASHBOARD_FETCH_CAP = 1000`, which is also PostgREST's default `max-rows`) ordered by `posted_at desc`, then ranks it in memory (`overall_score desc, nulls last, posted_at desc`). This is correct while the filtered result set is ≤ the cap — true for a single-user corpus. **If the active, filtered job set ever exceeds ~1000 rows, matches older than the cap could be excluded from ranking** (the fetch would return the 1000 most recent, not the 1000 highest-scored). The follow-up when the corpus approaches that size is a Postgres RPC/view that LEFT-joins `jobs` to their score and orders server-side with proper pagination. Until this fix (AD-49), the `.order` on the embedded column was a silent no-op and the dashboard was effectively sorted by recency, not match quality.
