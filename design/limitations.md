...
### 3.10 The Stored Eligibility Verdict Is An Ingest-Time Snapshot (`docs/decisions.md` AD-50)
**Resolved and replaced.** This section previously recorded that hard-excluded jobs were indistinguishable from AI-call failures on the row (AD-44): `classifyEligibility()`'s exclusions left `job_scores.ai_score` null with no other marker, so the dashboard reported them as "awaiting AI review" forever, and `findUnscored` re-queued them on every cron run. AD-50 fixed both by persisting the verdict as `jobs.ineligible_reason` at ingest.
