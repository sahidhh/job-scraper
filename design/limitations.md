...
### 3.10 Hard-Excluded Jobs Are Indistinguishable From AI-Call Failures On The Row (`docs/decisions.md` AD-44)
`classifyEligibility()`'s hard exclusions (geo-locked remote, sponsorship-refusing onsite) leave `job_scores.ai_score` null with no other row-level marker, by design (session scope: no schema changes). This is the exact same shape as a genuine transient AI-call failure (timeout/5xx/malformed response, §3.3) — both show up identically as "awaiting review" in the dashboard's pending stats and in `getScoringQueueReport()`'s `awaitingAiCount`. The only place the distinction is currently observable is the `scripts/score.ts` cron log line (`hard-excluded` vs `AI provider returned null`), not the dashboard or `/analytics`. A hard-excluded job is also not "stuck" in the AD-14/Phase-1-Task-6 sense — it will keep re-failing the same eligibility check on every retry pass rather than eventually succeeding — but nothing currently flags that difference either. Fixing this would need a new column/enum (e.g. a `job_scores.exclusion_reason` or a `not_eligible` value distinct from `null`), deliberately deferred past this session's no-migration constraint.
